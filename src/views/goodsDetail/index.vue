  ...
  <div class="prodetail">
    ...
    <div class="comment">
      <div class="comment-title">
        ...
        <div class="right" @click="getCommentList">查看更多 <van-icon name="arrow" /></div>
      </div>
    ...
    >
      <div class="product">
        ...
        <div class="showbtn" v-if="detail.stock_total > 0">
          <div class="btn" v-if="mode === 'cart'" @click="addFn">
            加入购物车
          </div>
          <div class="btn now" v-else @click="buyFn">立刻购买</div>
        </div>
...
import { getCommentData, getCommentList } from '@/api/comment'
...
export default {
  ...
  data () {
    return {
      ...
      total: null, // 评论总数
      defaultImg, // 默认头像图片
      showPanel: false, // 弹出框默认为false
      mode: '', // 弹出板模式
      count: 1,
      cartTotal: null // 购物车商品数量
    }
  },
  computed: {
    goodsId () {
  ...
  async created () {
    ...
    const {
      data: { list, total }
    } = await getCommentData(this.goodsId, 3)
    // console.log(res)
    this.list = list
    this.total = total
    // console.log(list)
  ...
  methods: {
    ...
    async getCommentList () {
      ...
      // console.log(res)
      this.$router.push(`/comment/${this.goodsId}`)
    }
