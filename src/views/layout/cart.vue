      ...
      <div class="cart-list">
        <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
          <van-checkbox
            ...
            @click="toggleChecked(item.goods_id)"
          ></van-checkbox>
          <div class="show">
          ...
          <div class="info">
            ...
            <span class="bottom">
              ...
              <!-- 保留自己的原数据，又希望调用函数传参，箭头函数 -->
              <CountBox
                :value="item.goods_num"
                @input="
                  (value) =>
                    changeCount(item.goods_id, value, item.goods_sku_id)
                "
              ></CountBox>
            </span>
          </div>
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          <div class="price">
            <span>合计：</span>
            <span
              >¥ <i class="totalPrice">{{ selTotalPrice }}</i></span
            >
          </div>
          <div
            v-if="!isEdit"
...
export default {
  ...
  methods: {
    ...
    handleDel () {
      if (this.selGoodsCount === 0) return
      this.$store.dispatch('cart/delCartAction')
      // this.$store.commit('cart/toggleAllChecked', false)
    },
    // 去结算
    goPay () {
      if (this.selGoodsCount > 0) {
        // 有选中的商品进行跳转
        this.$router.push({
          path: '/pay',
          query: {
